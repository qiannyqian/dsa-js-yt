...
function caesarCipher(str, shift) {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    //index of current character inside the alphabet array
    const idx = alphabetArr.indexOf(char);

    //if character is not in the array, indexOf will return -1
    if (idx === -1) {
      //add the character as it is
      res += char;
      //move to next character in the input string
      continue;
    }

    // use modulo to act as a ceiling
